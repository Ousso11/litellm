...
def _display_hash(hash_value: str) -> str:
    ...
    string, so cap what gets echoed into logs and the conversation."""
    return hash_value if len(hash_value) <= 32 else f"{hash_value[:32]}…"
...
def _scoped_store_key(logging_obj: object) -> Optional[str]:
...
def _assistant_text_from_response(response: object) -> Optional[str]:
...
class CompresrGuardrail(CustomGuardrail):
    ...
    def _retrieve_original(self, store_key: Optional[str], hash_value: str) -> str:
        if store_key:
    ...
    @staticmethod
    def _mirror_texts_channel(input_texts: object, applied: _CompressionResult) -> Optional[list[object]]:
